...
# (6 parámetros: dos esquinas opuestas del OBB, velocidad y ángulo) y el control
# (2 parámetros: aceleración y giro). Se puede descargar/copiar tal cual.
RUTAS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rutas")
DATASET_CSV = os.path.join(RUTAS_DIR, "dataset_apr_superv.csv")
...
# --------------------------------------------------------------------------- #
# Dataset de APRENDIZAJE SUPERVISADO
#
# De la RAMA DEFINITIVA (la trayectoria final ya volcada en veh.traj) se extrae,
# por cada vehículo y cada instante (paso DT), una muestra:
#   · ESTADO (6 parámetros de entrada): coordenadas de una esquina del OBB,
#     coordenadas de la esquina OPUESTA, velocidad (con signo) y ángulo (rad).
#   · CONTROL (2 parámetros de salida): aceleración (m/s²) y giro δ (rad).
# Velocidad, aceleración y giro se derivan de la propia trayectoria (posiciones
# a DT) con el modelo de bicicleta:
#     v      = desplazamiento proyectado sobre el rumbo / DT   (signo → marcha
#              adelante/atrás)
#     a      = (v[i+1] - v[i]) / DT
#     δ      = atan( L · Δθ / (v · DT) )   con L = batalla (wheelbase)
# El resultado se ACUMULA (modo append) en un único CSV descargable.
# --------------------------------------------------------------------------- #
def muestras_supervisadas(veh):
    """Filas (una por instante) de la trayectoria definitiva de 'veh':
    (vid, instante, x1, y1, x2, y2, v, theta, a, giro)."""
    traj = veh.traj
    n = len(traj)
    if n == 0:
        return []
    L = veh.wheelbase
    # Velocidad con signo en cada instante (0 en la pose final: aparcado).
    vel = [0.0] * n
    for i in range(n - 1):
        x, y, th = traj[i]
        dx = traj[i + 1][0] - x
        dy = traj[i + 1][1] - y
        vel[i] = (dx * math.cos(th) + dy * math.sin(th)) / DT
    filas = []
    for i in range(n):
        x, y, th = traj[i]
        c = obb_corners(x, y, th, veh.length, veh.width)
        (x1, y1), (x2, y2) = c[0], c[2]          # esquinas OPUESTAS del OBB
        v = vel[i]
        a = (vel[i + 1] - vel[i]) / DT if i < n - 1 else 0.0
        if i < n - 1 and abs(v) > 1e-6:
            dth = ang_norm(traj[i + 1][2] - th)
            giro = math.atan2(L * dth, v * DT)
        else:
            giro = 0.0
        filas.append((veh.vid, i, x1, y1, x2, y2, v, th, a, giro))
    return filas


def veh_a_dict(veh):
    """Reconstruye la especificación de 'veh' como el diccionario de ENTRADA del
    programa (mismo formato que la lista de diccionarios de texto; ángulos en
    GRADOS). Sirve para registrar las condiciones iniciales de cada generación."""
    ix, iy, th0 = veh.inicio
    mx, my = veh.meta
    ang = "libre" if veh.meta_th is None else round(math.degrees(veh.meta_th), 3)
    return {
        "id": veh.vid,
        "inicio": (round(ix, 3), round(iy, 3)),
        "giro_inicial": round(math.degrees(th0), 3),
        "meta": (round(mx, 3), round(my, 3)),
        "angulo_llegada": ang,
        "largo": round(veh.length, 3),
        "ancho": round(veh.width, 3),
        "v_max": round(veh.v_max, 3),
        "v_inicial": round(veh.v_inicial, 3),
        "a_max": round(veh.a_max, 3),
        "giro_max": round(math.degrees(veh.delta_max), 3),
        "grupo": veh.grupo,
        "prioridad": veh.prioridad,
    }
...
def guardar_dataset_supervisado(vehiculos, ruta=DATASET_CSV, opt="secuencial"):
    ...
    # Identificador de esta generación de rutas: distingue en el CSV acumulado las
    # muestras de una ejecución de las de otra (varios vehículos comparten 'run').
    run = time.strftime("%Y%m%d-%H%M%S") + f"-{int(time.time() * 1000) % 1000:03d}"
    # CONDICIONES INICIALES de la generación: la lista de diccionarios de entrada,
    # en una línea de COMENTARIO (#) propia —no ocupa columnas del CSV y los
    # lectores la ignoran con comment='#'—.
    condiciones = [veh_a_dict(v) for v in ok]
...
# --------------------------------------------------------------------------- #
# Presupuestos de búsqueda (compartidos por la GUI y el generador headless)
# --------------------------------------------------------------------------- #
