...
@njit(cache=True)
def _k_tiro(x, y, th, v, k, gx, gy, L, dmax, vmaxc, amax, goal_tol, v_tol, h,
...
class Planificador:
    ...
    def _construir_heuristica(self, gx, gy):
        """Campo de distancias desde la meta por Dijkstra 8-conexo sobre la rejilla
        libre: la longitud del camino más corto que rodea obstáculos, que orienta
        la búsqueda hacia rutas directas."""
    ...
    def _h_time(self, x, y):
        ...
        con respaldo euclídeo si la pose cae fuera de la rejilla."""
        res = self.h_res
        ix = int(round(x / res))
        iy = int(round(y / res))
        ...
        while abierto and expand < self.max_exp:
            ...
            if d_goal <= self.goal_tol and abs(v) <= self.v_tol:
                ...
                if abs(alpha0) <= self.ang_tiro or d_goal <= self.goal_tol * 1.5:
                    n, poses = _k_tiro(x, y, th, v, k, gx, gy, L, dmax,
...
def warmup():
    ...
    _k_tiro(2.0, 2.0, 0.0, 0.0, 0, 6.0, 6.0, 0.7, 0.6, 2.5, 1.0, 1.6, 0.45, 0.1,
